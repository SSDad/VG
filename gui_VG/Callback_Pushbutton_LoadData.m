...
%% load wave
[fn_Wave, dataPath] = uigetfile({'*Wave.mat'; '*.csv'}, 'Select a data file', fd_VG);
...
%         [tt, yy, baseLine] = fun_loadLoggerProData(fullfile(dataPath, fn_Wave));
    ...
    if mode(diff(tt)) ~= 1/pps
        tsin = timeseries(yy(:), tt(:));
        timevec = tsin.Timeinfo.Start:1/pps:tsin.Timeinfo.End;
    ...
    if isfield(data2.Panel.AllWave.Comp.hPlotObj, 'BoxB')
        ...
        data2.Panel.ViewB.Comp.hPlotObj.LAVBox.Position = [0 -100 0 0]; % LAV box
...
%             data2.Panel.TableBH.Comp.Radiobutton.Box(n).Value = 0;
        ...
        data2.Panel.ViewBH.Comp.hPlotObj.LAVBoxBH.Position = [0 -100 0 0]; % LAV box
        for n = 1:3
...
if fn_Analysis
    ...
    for iB = 1:length(avgB)
        ...
        for iW = 1:size(avgB(iB).wib.yy, 1)
            ...
            yJunk = avgB(iB).wib.yy(iW, :);
            yJunk = (yJunk - yA_mean.min)/range(yA_mean.data);
            line(avgB(iB).hg, 'XData', x, 'YData', yJunk, 'Color', BoxCLR);
        end
    end
